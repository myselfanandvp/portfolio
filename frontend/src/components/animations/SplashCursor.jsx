...
function SplashCursor({
  ...
  // Use a mutable ref to track changing config variables instantly without re-triggering context maps
  const configRef = useRef({
    ...
    SPLAT_FORCE, SHADING, COLOR_UPDATE_SPEED, BACK_COLOR, TRANSPARENT, RAINBOW_MODE, COLOR
  });
  ...
  useEffect(() => {
    ...
    let supportLinearFiltering = isWebGL2
    ...
    function updateColors(dt) {
      ...
      if (colorUpdateTimer >= 1) {
        colorUpdateTimer = ((colorUpdateTimer - 0) % 1) + 0;
    ...
    function applyInputs() {
      pointers.forEach(p => {
        if (p.moved) {
          p.moved = false;
          let dx = p.deltaX * configRef.current.SPLAT_FORCE;
          let dy = p.deltaY * configRef.current.SPLAT_FORCE;
    ...
    function render(target) {
      ...
      if (configRef.current.SHADING) gl.uniform2f(displayMaterial.uniforms.texelSize, 1.0 / width, 1.0 / height);
    ...
    function splat(x, y, dx, dy, color) {
      ...
      let radius = configRef.current.SPLAT_RADIUS / 100.0;
    ...
    function generateColor() {
      if (!configRef.current.RAINBOW_MODE) return hexToRGB(configRef.current.COLOR);
      let h = Math.random(), s = 1.0, v = 1.0;
    ...
    // Attach native viewport listeners
    window.addEventListener('mousemove', handleMouseMove, { passive: true });

    // Kick off runtime requestAnimationFrame loop
    animationFrameId = requestAnimationFrame(updateFrame);

    // Complete cleanup loop to remove background hooks, free hardware assets and stop simulation loops
    return () => {
      cancelAnimationFrame(animationFrameId);
      window.removeEventListener('mousemove', handleMouseMove);

      // Explicitly detach WebGL binding layers
      gl.bindBuffer(gl.ARRAY_BUFFER, null);
      gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, null);
      gl.bindFramebuffer(gl.FRAMEBUFFER, null);

      // Garbage collect GPU memory spaces
